...
CREATE TABLE `sys_device_alarm_data` (
  ...
  `water_depth` decimal(5,2) NOT NULL COMMENT '当前水面深度(米)，采用高精度BigDecimal类型处理',
  `status` tinyint(4) NOT NULL DEFAULT '0' COMMENT '异常状态标记：0-正常状态，1-十万火急溺水报警',
  `battery` tinyint(4) NOT NULL COMMENT '设备剩余电量百分比(0-100)',
  `report_time` datetime NOT NULL COMMENT '传感器报文上报时间(精确到毫秒级)',
  PRIMARY KEY (`id`),
  -- 核心高频范围查询索引：针对设备编码与时间段建立复合B+树索引，规避全表扫描，满足历史轨迹及范围调阅
  KEY `idx_device_time` (`device_code`, `report_time`),
  -- 溺水急救援警报极速拉取索引：独立过滤status状态，瞬间秒级捕获异常设备
